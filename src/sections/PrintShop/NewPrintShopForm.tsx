...
export const NewPrintShopForm = ({ topLevelTags, tagHierarchies, onAdd }: Props) => {
  const {
    control,
    handleSubmit,
    register,
  ...
  const onSubmit = (data: CreatePrintShop) => {
    ...
    axios
      ...
      .catch((error) => {
        console.error('Error adding print shop:', error);
      });
  };
  ...
  return (
    ...
    >
      <Typography variant="h4">새로운 인쇄소 추가</Typography>
      <Box
        sx={{
          display: 'grid',
      ...
      >
        ...
        <TextField
          {...register('homepage', {
            required: '홈페이지는 필수입니다.',
            pattern: {
              value: /^(https?:\/\/)?([\da-z.-]+)\.([a-z.]{2,6})([/\w.-]*)*\/?$/,
              message: '홈페이지 형식이 올바르지 않습니다.',
            },
          })}
          label="홈페이지"
        ...
        <TextField
          {...register('logoImage', {
            required: '로고 이미지 URL은 필수입니다.',
            pattern: {
              value: /^(https?:\/\/)?([\da-z.-]+)\.([a-z.]{2,6})([/\w.-]*)*\/?$/,
              message: '로고 이미지 URL 형식이 올바르지 않습니다.',
            },
          })}
          label="로고 이미지 URL"
        ...
        <TextField
          {...register('backgroundImage', {
            required: '배경 이미지 URL은 필수입니다.',
            pattern: {
              value: /^(https?:\/\/)?([\da-z.-]+)\.([a-z.]{2,6})([/\w.-]*)*\/?$/,
              message: '배경 이미지 URL 형식이 올바르지 않습니다.',
            },
          })}
          label="배경 이미지 URL"
        ...
        >
          {topLevelTags.map((topTag) => (
            <FormControl key={topTag.id} fullWidth error={Boolean(errors.tags)}>
